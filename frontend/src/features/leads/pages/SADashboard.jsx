...
const SADashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        const tasksRes = await fetch(`${apiBaseUrl}/tasks?agent_name=${encodeURIComponent(activeAgent)}`);
        const tasksData = await tasksRes.json();
        
        const actRes = await fetch(`${apiBaseUrl}/activities?agent_name=${encodeURIComponent(activeAgent)}`);
  ...
  const salesDoneCount = closedWonLeads.filter(l => l.sales_status === 'Sales Done').length;
  const salesDoneRevenue = closedWonLeads.filter(l => l.sales_status === 'Sales Done').reduce((sum, l) => sum + parseFloat(l.value || 0), 0);
      ...
      <div className="stats-row">
        ...
        <div className="stat-card purple">
          ...
          <div className="stat-info">
            ...
            <h3>{currencySymbol}{salesDoneRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</h3>
            <span className="stat-change">{salesDoneCount} deals finalized</span>
          </div>
        </div>
      </div>
