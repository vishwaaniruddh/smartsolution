...
const RevenueChart = ({ leads = [] }) => {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: '100%', height: '100%' }}>
      ...
      {values.map((v, i) => (
        <circle key={i} cx={toX(i)} cy={toY(v)} className="chart-dot" fill="#22d3ee" />
      ))}
    </svg>
  );
};
...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchLeads = fetch('http://localhost/lead/api/leads.php')
      .then(res => res.json())
      .catch(err => {
    ...
    const fetchUsers = fetch('http://localhost/lead/api/users.php')
      .then(res => res.json())
      .catch(err => {
    ...
    const fetchActivities = fetch('http://localhost/lead/api/activities.php')
      .then(res => res.json())
      .catch(err => {
