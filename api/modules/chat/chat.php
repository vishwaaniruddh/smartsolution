...
function trigger_socket_webhook($endpoint, $payload) {
    $webhook_url = 'http://localhost:3001/api/webhook/' . $endpoint;
    $payload['secret'] = 'dhurandhar_webhook_secret_handshake_2026!';
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 1); // Fast timeout so it doesn't block PHP if Node is down
    curl_exec($ch);
    curl_close($ch);
}
...
try {
    ...
    elseif ($action === 'history') {
        ...
        // Trigger socket webhook for read status update
        trigger_socket_webhook('mark-read', [
    ...
    elseif ($action === 'message' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Trigger socket webhook for real-time delivery
        trigger_socket_webhook('new-message', ['message' => $new_msg]);
